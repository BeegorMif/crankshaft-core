...
class AndroidAutoService : public QObject {
 ...
 public:
  ...
  /**
   * @brief Inject wireless network manager used by wireless AA implementations.
   *
   * Default implementation is a no-op so mock/legacy services do not need to
   * depend on network management details.
   */
  virtual void setWirelessNetworkManager(
  ...
  virtual auto setNegotiatedTouchResolution(const QSize& resolution) -> void = 0;
