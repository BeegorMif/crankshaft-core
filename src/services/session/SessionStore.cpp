...
bool SessionStore::initialize() {
  const QString dbDirectoryPath = QFileInfo(m_dbPath).absolutePath();
  if (!dbDirectoryPath.isEmpty()) {
    QDir dbDirectory(dbDirectoryPath);
    if (!dbDirectory.exists() && !dbDirectory.mkpath(QStringLiteral("."))) {
      Logger::instance().error(
          QString("[SessionStore] Failed to create database directory: %1").arg(dbDirectoryPath));
      return false;
    }
  }
